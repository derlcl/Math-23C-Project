...
Stable Distribution
KS Test / Chi-Squared Test (Chi-square Test on Varying Time Scales) / Complexities of Goodness of Fit Test
...
#Fourier Analysis
Before analyzing our data for properties of a random walk and infinite varience, we first need to run a Fourier Analysis to ensure there is no cyclic patterns in our data:
...
#Cauchy Distribution
The Cauchy Distribution is best known for it's undefined moment generating functions. As a result, the distribution has infinite variance which results in a high peak and long heavy tails.

Surprisngly, the distribution is only characterized by two parameters: location and scale which we esimated using the median and an innerquartile method used the $\textit{Goodness-of-Fit Testing for the Cauchy Distribution with Application to Financial Modeling}$ by M. Mahdizadeh, and Ehsan Zamanzade. Given the shape of the our data's histogram, and the underlying theoretical basis, we can investigate whether our data or the log of our data is well modeled by a Cauchy distribution:
```{r,echo=T}
...
#Implmenting the Stable Distribution
...
#Set Parameters for Cauchy and Stable:
#Daily Cuachy
diffs.median <- median(diffs) #3.429688
#Half Interquartile Range:
diffs.hiq <-  (quantile(diffs)[[4]] - quantile(diffs)[[2]]) /2
#Daily - Stable
stable.Xs <- quantile(diffs, c(.05, .25, .5, .75, .95))
#Calculate V's
stable.V_a <- (stable.Xs[[5]] - stable.Xs[[1]]) / (stable.Xs[[4]] - stable.Xs[[2]])
...
curve(dcauchy(x, location = diffs.median, scale =  diffs.hiq), add = TRUE, lwd = 3, col = "red")
curve(dstable(x, stable.a, stable.b, stable.c, stable.location), add = TRUE, lwd = 2, col = "green")
...
The hurst exponent is moving away from .5 as the data set accounts for broader periods of time. In other words, the stock prices show more mild randomness as we go from daily stock price changes to yearly stock price changes. 


###########################

#To do:
#Comment about their hurst exponents
#Swtich to Goodness of Fit discussions
#Add legends to histo grams if possible lol
#Tie together all the other parts of the report
#Finish bibliography
#Clean up the script to match the report



#Simulated Cuachy Pvalue test 
This appears to be a good fit. But let us see if it stands up to goodness of fitness tests:
```{r, echo=T}
...
cauchy.pvalue <- mean(results.Cauchy >= cauchy.cs); cauchy.pvalue
```
This gives us a P value of approximately 24%. Therefore, we fail to reject the null hypothesis. There is significant evidence to suggest that our data pulls from (at the very least) a family-member of heavy tail stable distributions. Although it is definitely not a gaussian distribution. It may or may not be Cauchy, but since all Stable distributions besides Gaussian have infinite variance, it appears our data's distribution also has infinite variance.

As a robustness test, we perform the same analysis using Octiles:
```{r,echo=T}
hist(diffs, prob = TRUE, breaks = "FD", main = "Histogram of First Differences
     Cauchy Model", xlab = "First Differences")
#Median:
...
cauchy.pvalue <- mean(results.Cauchy >= cauchy.cs); cauchy.pvalue
```
And we get P value of approximately 12%. Thus, we fail to reject the null hypothesis again, and gain some more confidence that our data is well modeled by some variation of a Cauchy distribution.
...
{hist(rand, probability = TRUE, breaks = "FD", main = "   Histogram of One Random Cauchy Sample 
      Overlayed by Cauchy Estimated Parameters", xlab = "rCauchy Values")
...
```
It appears that the results are favorable to the hypothesis that the data matches a Cauchy distribution, since 72% of the time the p-value from the K-S test was less than 0.05.

Now we'll try sing fitdist parameters:
```{r,echo=T, warning = F}
rand2 <- rcauchy(nn, location = fit.diffs[1], scale = fit.diffs[2]); head(rand)
hist(rand2, probability = TRUE, breaks = "FD", main = "   Histogram of One Random Cauchy Sample 
      Overlayed by Cauchy Estimated Parameters", xlab = "rCauchy Values")
...
abline(v = 0.05, col = "red")}
```
And we get even better results here! As about 92% of p-values are greater than 0.05, thus we have some convincing evidence in favor of the null hypothesis that the samples come from similar distributions.

#As a robustness test, let's compare samples taken from the data to samples taken from a Cauchy distribution
Taking N samples from diffs and using fitdist to fit each of those. Then comparing each of those samples to a Cauchy distribution with those same parameters, and then taking chi-squares:
```{r,echo=T, warning = FALSE}
N <- 10^3
samp.rand <- numeric(N)
nn <- 500
...
#Now to show the impact of sample sizes on the K-S test:
This script does take a long time to run, but unfortunatly decreasing the number of samples being drawn compromises the completeness of the plot.
```{r,echo=T, warning=F}
N <- 6000
ks.stat <- numeric(N)
for (i in 1:N) {
...
```
And what we can see is that the K-S test statistic is sensitive to the size of the random sample being drawn from the random Cauchy distribution. We conclude that this is a result of the fact that the diffs samples are drawn from a sample that has finite data (DJI), whereas the rcauchy samples are drawn from the true distribution, which has infinite variance and infinitely many data points. We note however, that samples with as many as 500 elements have provided good evidence that we should not reject the null hypothesis.
