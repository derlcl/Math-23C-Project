...
```{r,echo=T, warning=FALSE, error = TRUE}
nn <- 500 #sample size. 
...
for (i in 1:N) {
  ...
  ks.stats2[i] <- ks.test(unique(diff.samp2),unique(rand2))$p.value
}
...
for(i in 1:N){
  ...
  ks.samp.rand[i] <- ks.test(unique(diff.samp), unique(cauchy.samp))$p.value
}
